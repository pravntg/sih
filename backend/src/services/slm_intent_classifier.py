...
2. Comprehensive Global & Indian Coastal Gazetteer (Dapoli, Ratnagiri, Malvan, Alibaug, Karwar, etc.).
...
# Comprehensive Coastal Gazetteer of 120+ Major & Minor Coastal Ports/Landing Centres
...
class SLMIntentClassifier:
    ...
    def resolve_coastal_entity(cls, text: str) -> Optional[Tuple[float, float, str, str]]:
        """
        Resolves coastal city, port, or landmark in text against the 120+ Coastal Gazetteer.
        """
        text_lower = text.lower()
        tokens = set(re.findall(r'[a-zA-Z0-9]+', text_lower))

        # Check multi-word locations first (e.g. 'port blair', 'sagar island')
